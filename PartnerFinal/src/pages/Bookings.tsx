...
import { BanIcon, BellRingIcon, CheckIcon, DownloadIcon, LogInIcon, XIcon } from 'lucide-react';
...
export function Bookings() {
  ...
  /** Live channel stand-in: a new extension request lands without a page refresh. */
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      booking.id === 'CHK2447' ?
      {
        ...
        extension: {
          extraHours: 3,
          currentCheckout: booking.checkOutTime,
          requestedCheckout: '2:00 AM',
          ...
          capacity: {
            available: true,
            note: '12 of 18 allocated slots sold in this window.'
          }
        }
      } :
      booking
  ...
  const pendingExtensions = useMemo(
    () => rows.filter((booking) => booking.extension?.state === 'Pending Approval'),
    [rows]
  );
  ...
  const visible = useMemo(
    ...
    /** Rows with a live extension request float to the top for faster action. */.
    sort((a, b) => {
      const score = (booking: Booking) =>
      booking.extension?.state === 'Pending Approval' ? 0 : 1;
    ...
    switch (action) {
      ...
      case 'accept':
        ...
        addAudit({
          ...
          detail: `#${booking.id} • ${slotLabel(booking.room)}`,
          category: 'Operations'
        });
        break;
      case 'reject':
      ...
      case 'checkout':
        ...
        notify(`${booking.guest} checked out — slot released for housekeeping`);
        addAudit({
  ...
  const exportCsv = () => {
    ...
    booking.status,
    booking.extension ? booking.extension.state : 'None'].
  ...
  const patchExtension = (id: string, changes: Partial<BookingExtension>, timelineLabel: string) => {
    setRows((prev) =>
    prev.map((booking) =>
    booking.id === id && booking.extension ?
    {
      ...booking,
      extension: { ...booking.extension, ...changes },
      timeline: [
      ...booking.timeline.map((entry) => ({ ...entry, done: true })),
      {
        id: `x${Date.now()}-${booking.id}`,
        label: timelineLabel,
        time: 'Just now',
        done: true
      }]
  ...
  const approveExtension = (booking: Booking) => {
    const extension = booking.extension;
    if (!extension) return;
    if (!can('accept_bookings')) {
      notify(`${role?.name} role cannot approve extensions — permission required`);
      return;
    }
    setRows((prev) =>
    prev.map((item) =>
    item.id === booking.id && item.extension ?
    {
      ...item,
      checkOutTime: extension.requestedCheckout,
      extension: { ...item.extension, state: 'Approved', respondWithin: 0 },
      timeline: [
      ...item.timeline.map((entry) => ({ ...entry, done: true })),
      {
        id: `x${Date.now()}-${item.id}`,
        label: `Extension approved (+${extension.extraHours} hours)`,
        time: 'Just now',
        done: true
      }]
    ...
    notify(
      `✅ Extension Approved Successfully — checkout now ${extension.requestedCheckout}, ${booking.guest} notified`
    );
    addAudit({
      action: 'Approved stay extension',
      detail: `#${booking.id} • ${slotLabel(booking.room)} • +${extension.extraHours}h • ${inr(extension.additionalRevenue)}`,
      category: 'Operations'
    });
  };

  const rejectExtension = (booking: Booking, reason: string) => {
    if (!can('reject_bookings')) {
      notify(`${role?.name} role cannot reject extensions — permission required`);
      return;
    }
    patchExtension(
      booking.id,
      { state: 'Rejected', rejectionReason: reason, respondWithin: 0 },
      `Extension rejected — ${reason}`
    );
    notify(`❌ Extension Request Rejected — ${booking.guest} notified, checkout unchanged`);
    addAudit({
      action: 'Rejected stay extension',
      detail: `#${booking.id} • ${reason}`,
      category: 'Operations'
    });
  };

  const expireExtension = (id: string) => {
    setRows((prev) =>
    prev.map((booking) =>
    booking.id === id && booking.extension?.state === 'Pending Approval' ?
    { ...booking, extension: { ...booking.extension, state: 'Expired', respondWithin: 0 } } :
    booking
    )
    );
  };
  ...
  const incomingBooking = rows.find((booking) => booking.id === incoming) ?? null;
  ...
  return (
    <main className="scroll-slim h-full flex-1 overflow-y-auto bg-canvas px-7 py-6">
      ...
      <div className="mt-6 space-y-5 pb-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-orange-300 bg-orange-50 px-4 py-3">
            <div className="min-w-0">
              <p className="flex items-center gap-2 text-[13.5px] font-semibold text-ink">
                ...
                {pendingExtensions.length} extension request
                {pendingExtensions.length > 1 ? 's' : ''} waiting for approval
              </p>
              <p className="mt-0.5 text-[12.5px] text-ink-soft">
                {inr(
                pendingExtensions.reduce(
                  (sum, booking) => sum + (booking.extension?.additionalRevenue ?? 0),
                  0
                )
              )}{' '}
                additional revenue at stake — these rows are pinned to the top of the table.
              </p>
            </div>
            <button
            type="button"
            onClick={() => setOpenId(pendingExtensions[0].id)}
            className="rounded-xl bg-ink px-4 py-2.5 text-[13px] font-semibold text-white transition-colors duration-150 ease-out hover:bg-ink-soft">
            
              Review oldest request
            </button>
          </div>
        }
        ...
        <BookingsTable
          ...
          onExtensionExpire={expireExtension} />
      ...
      <BookingDetailsDrawer
        ...
        onExpire={handleExpire}
        onApproveExtension={approveExtension}
        onRejectExtension={rejectExtension}
        onExtensionExpire={expireExtension} />
      

      {incomingBooking?.extension &&
      <aside
        role="status"
        className="fixed right-6 top-[84px] z-[60] w-[320px] rounded-2xl border border-neutral-200 bg-white/95 p-4 shadow-lg backdrop-blur">
        
          <div className="flex items-start gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-orange-100">
              <BellRingIcon size={17} className="text-orange-600" aria-hidden="true" />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-[13.5px] font-semibold text-ink">New Extension Request Received</p>
              <p className="mt-0.5 text-[12.5px] text-ink-soft">
                {incomingBooking.guest} requests +{incomingBooking.extension.extraHours} hours.
              </p>
              <p className="mt-0.5 text-[12px] text-ink-muted">
                {slotLabel(incomingBooking.room)} • Booking {incomingBooking.id}
              </p>
              <button
              type="button"
              onClick={() => {
                setOpenId(incomingBooking.id);
                setIncoming(null);
              }}
