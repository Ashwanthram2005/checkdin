...
export function AuthModal() {
  const { authOpen, closeAuth, signIn, signInWithProvider } = useAuth();
  ...
  async function sendOtp() {
    ...
    setLoading(true);
    try {
      await signIn({ phone, name: name.trim() || undefined });
      reset();
    } catch (err: unknown) {
  ...
  async function emailContinue() {
    ...
    setLoading(true);
    try {
      await signIn({ email: email.trim(), name: name.trim() });
      reset();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Login failed. Please try again.';
      setError(msg);
  ...
  return (
    <Modal open={authOpen} title={title} onClose={close}>
      ...
      <div>
          ...
          onClick={sendOtp}
          disabled={loading}
          className="mt-5 w-full rounded-xl bg-primary px-5 py-3.5 text-sm font-bold text-ink transition-colors duration-150 ease-smooth hover:bg-primary-dark disabled:opacity-70">
