...
import {
  ...
  ZapIcon } from
'lucide-react';
import { ScrollSearchBar } from '../components/search/ScrollSearchBar';
...
export function Home() {
  ...
  useEffect(() => {
    loadHotels({ limit: 4 }).then(({ hotels }) => setFeatured(hotels));
  }, []);

  return (
    <div className="w-full">
      <ScrollSearchBar anchorRef={heroSearchRef} onMeasure={setBarHeight} />
      {/* Hero band */}
      <section className="relative overflow-hidden bg-night text-white">
        ...
        <div className="relative mx-auto w-full max-w-[1360px] px-5 pb-6 pt-12 lg:px-8">
          ...
          {/* Reserves the search bar's starting slot — the bar itself is
               scroll-linked and rendered in a fixed, centered shell. */}
          <div
            ref={heroSearchRef}
            aria-hidden="true"
