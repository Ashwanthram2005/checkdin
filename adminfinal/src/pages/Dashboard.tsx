...
export function Dashboard() {
  ...
  /** Live AdminOS queues that need a decision from someone today. */
  const osQueues = [
  {
    label: 'Extensions awaiting a decision',
    value: os.extensions.filter((row) => row.status === 'Pending').length,
    to: '/os/extensions?status=Pending',
    urgent: false
  },
  {
    label: 'Hotels not visible to guests',
    value: os.visibility.filter((row) => row.state !== 'Live').length,
    to: '/os/hotel-status',
    urgent: false
  },
  {
    label: 'Settlements unreleased',
    value: os.settlements.filter((row) => row.status === 'Pending' || row.status === 'On Hold').length,
    to: '/os/settlements',
    urgent: false
  },
  {
    label: 'Open disputes',
    value: os.disputes.filter((row) => row.status === 'Open' || row.status === 'In Review').length,
    to: '/os/disputes',
    urgent: false
  },
  {
    label: 'Compliance blockers',
    value: os.compliance.filter((row) => row.stage === 'Onboarding' || row.stage === 'Suspended').length,
    to: '/os/compliance',
    urgent: false
  },
  {
    label: 'High-risk entities',
    value: os.risks.filter((row) => row.band === 'High' && row.status !== 'Cleared').length,
    to: '/os/risk',
    urgent: true
  }];
  ...
  return (
    <div>
      ...
      <Card className="mb-4 border-accent/60 bg-accent/[0.07]">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 px-5 py-4">
          <div className="min-w-0">
            <p className="text-[15px] font-semibold tracking-tight text-ink">Checkdin AdminOS</p>
            <p className="mt-0.5 text-[13px] text-muted">
              Control. Monitor. Scale. — marketplace intelligence, extensions, settlements, and platform governance.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 sm:ml-auto">
            <Link to="/os">
              <Button size="sm" variant="primary">
                Platform overview <ArrowRightIcon className="h-3.5 w-3.5" />
              </Button>
            </Link>
            <Link to="/os/command-center">
              <Button size="sm">Command center</Button>
            </Link>
            <Link to="/os/ai-insights">
              <Button size="sm">AI insights</Button>
            </Link>
          </div>
        </div>
      </Card>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader
              title="AdminOS status"
              subtitle={
              osAttention === 0 ?
              'Every platform queue is clear' :
              `${osAttention} items across ${osQueues.filter((queue) => queue.value > 0).length} queues need a decision`
              }
              action={
              <Link to="/os">
                  <Button size="sm" variant="ghost">
            ...
            <ul className="divide-y divide-line">
              ...
              <li key={queue.label}>
                  <Link
                  to={queue.to}
                  className="flex items-center gap-3 px-5 py-2.5 transition-colors duration-150 ease-smooth hover:bg-faint">
                  
                    <span className="min-w-0 flex-1 truncate text-[13px] text-ink">{queue.label}</span>
                    <span
                    className={`text-[15px] font-bold tabular-nums ${
                    queue.value === 0 ? 'text-muted' : queue.urgent ? 'text-negative' : 'text-ink'}`
                    }>
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-line px-5 py-3">
              <Badge tone={osUnread > 0 ? 'warning' : 'neutral'}>{osUnread} unread alerts</Badge>
              <Link to="/os/notifications" className="ml-auto">
                <Button size="sm" variant="ghost">
                  Open inbox
                </Button>
              </Link>
